...
void DepthImageRenderer::initContext(GLContextData& contextData) const
{
        ...
        for (unsigned int y = 0; y < depthImageSize[1]; ++y)
            for (unsigned int x = 0; x < depthImageSize[0]; ++x, ++vPtr)
            {
                ...
                Kinect::LensDistortion::Point dp(
                    Kinect::LensDistortion::Scalar(x) + Kinect::LensDistortion::Scalar(0.5),
    ...
    glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, (depthImageSize[1] - 1) * depthImageSize[0] * 2 * sizeof(GLuint), 0,
                    GL_STATIC_DRAW_ARB);
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_LUMINANCE32F_ARB, depthImageSize[0], depthImageSize[1], 0,
    ...
    dataItem->depthShaderUniforms[1] =
        glGetUniformLocationARB(dataItem->depthShader, "projectionModelviewDepthProjection");
    ...
    dataItem->elevationShaderUniforms[3] =
        glGetUniformLocationARB(dataItem->elevationShader, "projectionModelviewDepthProjection");
}
...
void DepthImageRenderer::bindDepthTexture(GLContextData& contextData) const
{
    ...
    if (dataItem->depthTextureVersion != depthImageVersion)
    {
        ...
        glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, depthImageSize[0], depthImageSize[1], GL_LUMINANCE, GL_FLOAT,
                        depthImage.getData<GLfloat>());
