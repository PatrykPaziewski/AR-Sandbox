...
GLhandleARB SurfaceRenderer::createSinglePassSurfaceShader(const GLLightTracker& lt, GLint* uniformLocations) const
{
    ...
    try
    {
...
#if defined(SAVE_SHADERS)
        std::ofstream file;
        file.open("shader.vs");
        file.write(vertexFunctions.c_str(), vertexFunctions.size());
        file.write("\t\t\n", 3);
        file.write(vertexUniforms.c_str(), vertexUniforms.size());
        file.write("\t\t\n", 3);
        file.write(vertexVaryings.c_str(), vertexVaryings.size());
        file.write("\t\t\n", 3);
        file.write(vertexMain.c_str(), vertexMain.size());
        file.close();
#endif
        ...
        *********************************************************************/
        ...
        if (drawSlopes)
        {
            /* Declare the slope coloring function: */
            fragmentDeclarations += "\
                void drawSlopes(in vec2,inout vec4);\n";

            /* Compile the slope colouring shader: */
            shaders.push_back(compileFragmentShader("SurfaceSlopeShader"));
            ...
            fragmentMain += "\
                ...
                drawSlopes(gl_FragCoord.xy,baseColor);\n\
                \n";
        }

        if (drawAspect)
        {
            /* Declare the aspect coloring function: */
            fragmentDeclarations += "\
                void drawAspect(in vec2,inout vec4);\n";

            /* Compile the aspect colouring shader: */
            shaders.push_back(compileFragmentShader("SurfaceAspectShader"));

            /* Call aspect coloring function from fragment shader's main function: */
            fragmentMain += "\
                /* Modulate the base color using gradient descend/ascend angle: */\n\
                drawAspect(gl_FragCoord.xy,baseColor);\n\
                \n";
        }
...
#if defined(SAVE_SHADERS)
        file.open("shader.fs");
        file.write(fragmentDeclarations.c_str(), fragmentDeclarations.size());
        file.write("\t\t\n", 3);
        file.write(fragmentUniforms.c_str(), fragmentUniforms.size());
        file.write("\t\t\n", 3);
        file.write(fragmentVaryings.c_str(), fragmentVaryings.size());
        file.write("\t\t\n", 3);
        file.write(fragmentMain.c_str(), fragmentMain.size());
        file.close();
#endif
...
void SurfaceRenderer::renderPixelCornerElevations(const int viewport[4], const PTransform& projectionModelview,
                                                  GLContextData& contextData, SurfaceRenderer::DataItem* dataItem) const
{
        ...
        dataItem->contourLineFramebufferSize[1] != (unsigned int)(viewport[3] + 1))
    {
        ...
        bool mustAttachBuffers =
            dataItem->contourLineFramebufferSize[0] == 0 && dataItem->contourLineFramebufferSize[1] == 0;
        ...
        glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT, dataItem->contourLineFramebufferSize[0],
    ...
    , animationTime(0.0)
{
    ...
    fileMonitor.addPath((std::string(CONFIG_SHADERDIR) + std::string("/SurfaceAddContourLines.fs")).c_str(),
    ...
    fileMonitor.addPath((std::string(CONFIG_SHADERDIR) + std::string("/SurfaceIlluminate.fs")).c_str(),
                        IO::FileMonitor::Modified,
                        Misc::createFunctionCall(this, &SurfaceRenderer::shaderSourceFileChanged));
    fileMonitor.addPath((std::string(CONFIG_SHADERDIR) + std::string("/SurfaceAddWaterColor.fs")).c_str(),
                        IO::FileMonitor::Modified,
                        Misc::createFunctionCall(this, &SurfaceRenderer::shaderSourceFileChanged));
    fileMonitor.addPath((std::string(CONFIG_SHADERDIR) + std::string("/SurfaceSlopeShader.fs")).c_str(),
                        IO::FileMonitor::Modified,
                        Misc::createFunctionCall(this, &SurfaceRenderer::shaderSourceFileChanged));
    fileMonitor.addPath((std::string(CONFIG_SHADERDIR) + std::string("/SurfaceAspectShader.fs")).c_str(),
                        IO::FileMonitor::Modified,
                        Misc::createFunctionCall(this, &SurfaceRenderer::shaderSourceFileChanged));
    fileMonitor.addPath((std::string(CONFIG_SHADERDIR) + std::string("/SurfaceHillshadeShader.fs")).c_str(),
                        IO::FileMonitor::Modified,
                        Misc::createFunctionCall(this, &SurfaceRenderer::shaderSourceFileChanged));
...
void SurfaceRenderer::initContext(GLContextData& contextData) const
{
    ...
    dataItem->heightMapShader =
        createSinglePassSurfaceShader(*contextData.getLightTracker(), dataItem->heightMapShaderUniforms);
    ...
    dataItem->globalAmbientHeightMapShaderUniforms[0] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "depthSampler");
    dataItem->globalAmbientHeightMapShaderUniforms[1] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "depthProjection");
    dataItem->globalAmbientHeightMapShaderUniforms[2] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "basePlane");
    dataItem->globalAmbientHeightMapShaderUniforms[3] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "pixelCornerElevationSampler");
    dataItem->globalAmbientHeightMapShaderUniforms[4] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "contourLineFactor");
    dataItem->globalAmbientHeightMapShaderUniforms[5] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "heightColorMapSampler");
    dataItem->globalAmbientHeightMapShaderUniforms[6] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "heightColorMapTransformation");
    dataItem->globalAmbientHeightMapShaderUniforms[7] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "waterLevelSampler");
    dataItem->globalAmbientHeightMapShaderUniforms[8] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "waterLevelTextureTransformation");
    dataItem->globalAmbientHeightMapShaderUniforms[9] =
        glGetUniformLocationARB(dataItem->globalAmbientHeightMapShader, "waterOpacity");
    ...
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[0] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "depthSampler");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[1] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "depthProjection");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[2] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "tangentDepthProjection");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[3] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "basePlane");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[4] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "pixelCornerElevationSampler");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[5] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "contourLineFactor");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[6] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "heightColorMapSampler");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[7] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "heightColorMapTransformation");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[8] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "waterLevelSampler");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[9] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "waterLevelTextureTransformation");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[10] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "waterOpacity");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[11] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "shadowTextureSampler");
    dataItem->shadowedIlluminatedHeightMapShaderUniforms[12] =
        glGetUniformLocationARB(dataItem->shadowedIlluminatedHeightMapShader, "shadowProjection");
}
...
void SurfaceRenderer::setDrawSlopes(bool newDrawSlopes)
{
    drawSlopes = newDrawSlopes;
    ++surfaceSettingsVersion;
}

void SurfaceRenderer::setDrawAspect(bool newDrawAspect)
{
    drawAspect = newDrawAspect;
    ++surfaceSettingsVersion;
}
                                       ...
                                       const OGTransform& modelview, GLContextData& contextData) const
{
    ...
    if (dataItem->surfaceSettingsVersion != surfaceSettingsVersion ||
        (illuminate && dataItem->lightTrackerVersion != contextData.getLightTracker()->getVersion()))
    {
        /* Rebuild the shader: */
        try
        {
        ...
        catch (const std::runtime_error& err)
        {
            Misc::formattedUserError(
                "SurfaceRenderer::renderSinglePass: Caught exception %s while rebuilding surface shader", err.what());
        }
