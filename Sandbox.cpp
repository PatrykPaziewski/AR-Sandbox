...
void Sandbox::rawDepthFrameDispatcher(const Kinect::FrameBuffer& frameBuffer)
{
    ...
    if (saver != 0)
        saver->saveDepthFrame(frameBuffer);
}
...
void Sandbox::receiveFilteredFrame(const Kinect::FrameBuffer& frameBuffer)
{
    ...
    if (saver != 0)
        saver->saveColorFrame(frameBuffer);
...
GLMotif::PopupWindow* Sandbox::createWaterControlDialog(void)
{
    ...
    GLMotif::PopupWindow* waterControlDialogPopup =
        new GLMotif::PopupWindow("WaterControlDialogPopup", Vrui::getWidgetManager(), "Water Simulation Control");
    ...
    GLMotif::RowColumn* waterControlDialog =
        new GLMotif::RowColumn("WaterControlDialog", waterControlDialogPopup, false);
    ...
    waterMaxStepsSlider =
        new GLMotif::TextFieldSlider("WaterMaxStepsSlider", waterControlDialog, 8, ss.fontHeight * 10.0f);
    ...
    waterAttenuationSlider =
        new GLMotif::TextFieldSlider("WaterAttenuationSlider", waterControlDialog, 8, ss.fontHeight * 10.0f);
...
void printUsage(void)
{
    ...
    std::cout << "     Save color/depth input from Kinect and save it to file with given name" << std::endl;
}
    ...
    , controlPipeFd(-1)
{
    ...
    Math::Interval<double> elevationRange =
        cfg.retrieveValue<Math::Interval<double>>("./elevationRange", Math::Interval<double>(-1000.0, 1000.0));
    ...
    Math::Interval<double> rainElevationRange =
        cfg.retrieveValue<Math::Interval<double>>("./rainElevationRange", Math::Interval<double>(-1000.0, 1000.0));
    ...
    if (frameFilePrefix != 0)
    {
        ...
        camera =
            new Kinect::FileFrameSource(Vrui::openFile(colorFileName.c_str()), Vrui::openFile(depthFileName.c_str()));
    }
    ...
    else
    {
        ...
        /* Open the selected pre-recorded 3D video files: */
        std::string colorFileName = saveFilePrefix;
        colorFileName.append(".color");
        std::string depthFileName = saveFilePrefix;
        depthFileName.append(".depth");
        saver = new Kinect::FrameSaver(*realCamera, Vrui::openFile(colorFileName.c_str(), IO::File::WriteOnly),
                                       Vrui::openFile(depthFileName.c_str(), IO::File::WriteOnly));
    }
    for (int i = 0; i < 2; ++i)
    ...
    for (auto& rsELem : renderSettings)
    {
        if (rsELem.elevationColorMap != 0)
            rsELem.elevationColorMap->setScalarRange(rsELem.elevationColorMap->getScalarRangeMin() * sf,
                                                     rsELem.elevationColorMap->getScalarRangeMax() * sf);
        rsELem.contourLineSpacing *= sf;
        rsELem.waterOpacity /= sf;
        for (int i = 0; i < 4; ++i)
            rsELem.projectorTransform.getMatrix()(i, 3) *= sf;
    }
    rainStrength *= sf;
    evaporationRate *= sf;
    demDistScale *= sf;
    ...
    frameFilter =
        new FrameFilter(frameSize, numAveragingSlots, pixelDepthCorrection, cameraIps.depthProjection, basePlane);
    ...
    for (auto& rsElem : renderSettings)
    {
        ...
        {
            if (haveHeightMapPlane)
                rsElem.elevationColorMap->calcTexturePlane(heightMapPlane);
            else
                rsElem.elevationColorMap->calcTexturePlane(depthImageRenderer);
        }
        ...
        if (waterTable != 0)
        {
            if (rsElem.renderWaterSurface)
            {
                /* Create a water renderer: */
                rsElem.waterRenderer = new WaterRenderer(waterTable);
            }
            else
            {
                rsElem.surfaceRenderer->setWaterTable(waterTable);
                rsElem.surfaceRenderer->setAdvectWaterTexture(true);
                rsElem.surfaceRenderer->setWaterOpacity(rsElem.waterOpacity);
            }
        }
        rsElem.surfaceRenderer->setDemDistScale(demDistScale);
    }
            ...
            while (*cPtr != '\0')
            {
                ...
                else if (isToken(tokens[0], "dippingBed"))
                {
                    ...
                    else if (tokens.size() == 5)
                    {
                        ...
                        SurfaceRenderer::Plane dippingBedPlane =
                            SurfaceRenderer::Plane(SurfaceRenderer::Plane::Vector(dbp), dbp[3]);
...
{
    /* Construct a navigation transformation to center the sandbox area in the display, facing the viewer, with the long
     * sandbox axis facing to the right: */
